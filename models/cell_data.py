...
class GradeCountDataBuilder():
    ...
    def build(self, row:int, count:int):
        ...
        width = 50
        height = 48
        bg_colour   = self.__extract_background_colour(row, count)
        text_colour = self.__extract_text_colour(row, count)

        return GradeCountData(row, width, height, bg_colour, text_colour, str(count))

    def __extract_background_colour(self, row:int, count:int):
